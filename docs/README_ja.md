...
| `KIROK_reflect` | 蓄積された記憶を分析して洞察を生成、自動更新可能なメンタルモデルとして保存 |
| `KIROK_smart_retain` | 内容の重要度を評価し、基準以上なら通常の記憶保存処理を実行（大量データ向け） |
...
| `KIROK_clear_bank` | バンク内の全記憶とパターン検出結果を削除 |
...
| `KIROK_RECALL_MIN_SIMILARITY` | ❌ | `0.62` | 想起で意味検索の記憶を採用する最低コサイン類似度（キーワード検索のヒットは対象外） |
| `KIROK_OBS_MIN_SIMILARITY` | ❌ | `0.62` | 想起で意味検索のパターン検出結果を採用する最低コサイン類似度 |
| `KIROK_AUTO_SNAPSHOT_HOURS` | ❌ | `24` | 起動時の自動バックアップの間隔（時間）（`0` で無効化） |
| `KIROK_SNAPSHOT_KEEP` | ❌ | `5` | 自動バックアップを何世代残すか |
...
`--online` を付けると、実際に 1 回だけ埋め込み API を呼び出して Gemini
への疎通を確認します（`GEMINI_API_KEY` を使用）。既定の確認項目はオフラインのままです：
...
手動コマンドに加えて、サーバー起動時にも**自動バックアップ**を取ります。
起動のたびに、直近の自動バックアップが `KIROK_AUTO_SNAPSHOT_HOURS`
（既定 24 時間。`0` で無効化）より古ければ、`~/.kirok/backups/memory-auto-*.db`
へ複製を保存し、`KIROK_SNAPSHOT_KEEP`（既定 5 世代）を超えた古い世代を
削除します。`kirok-backup snapshot` を手動で実行し忘れた場合の保険であり、
手動バックアップや書き出しファイルには一切触れません。
...
1. **記憶（Retain）**：情報を保存すると Kirok は以下を自動実行します
   ...
   - 十分な数の記憶が溜まるとパターン検出（consolidation）を自動実行します。
     この処理は単一トランザクションとして原子的に適用され、LLM が不要と
     判断したパターン検出結果は完全削除せず `deprecated_at` を付けて
     ソフト削除します（検索・統計からは除外されつつ、行自体は復旧用に残る）
...
3. **考察（Reflect）**：考察を実行すると Kirok は以下を実行します
   ...
   - 結果を新しいメンタルモデルとして保存
   - 必要に応じて、今後のパターン検出後に自動更新されるよう設定

### メモリバンクとは？

記憶は **メモリバンク** に分類して保存します。フォルダのようなものと考えてください。
想起の精度を保つため、用途ごとに細かくバンクを乱立させず、少数の安定した
汎用バンクにまとめることを推奨します（詳細は `skills/kirok/SKILL.md` の
Kirok スキル向けガイドを参照）：

- `"user-prefs"` — ユーザーの個人的な好み・ルール
- `"architecture"` — システム設計・技術仕様
- `"troubleshooting"` — エラーの記録・原因・対処法
- `"milestones"` — プロジェクトの達成事項・作業履歴
- `"scratch"` — 一時的・揮発性のメモ

新しいバンクを増やすのは、その用途が今後多くの記憶を受け止められるほど
広い場合に限ります。AIエージェントが使っていくうちに、適切なバンク名を提案してくれます。
