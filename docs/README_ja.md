...
## 🎁 特典：Agent Skills スターターパック同梱

Kirokの記憶力を最大限に引き出すため、**プロフェッショナルの手順をAIに教え込む「Agent Skills」の厳選日本語化セット（5種類）** を同梱しています！

- `kirok`：AI自身にKirokの操作方法とベストプラクティスを教える基本スキル
- `friendly-guide`：AIの返答を「IT用語や英語を使わない、分かりやすい日本語」に強制するスキル
- `doc-coauthoring`：AIと協力して高品質なドキュメントを作成するためのスキル
- `skill-creator`：AIに新しい「Agent Skills」をゼロから作らせるスキル
- `ceremonial-rescue-jp`：日本の冠婚葬祭の作法やマナーを完璧にサポートするデモ・スキル

**使い方**：ダウンロードした `skills` フォルダを、あなたの作業ディレクトリに置くだけで、あなたのAIが瞬時にこれらの専門知識を獲得します。
